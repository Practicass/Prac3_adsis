#!/bin/bash
#842255, Angusto Delgado, Pablo Ernesto, T, 1, A
#850068, Arejula Aisa, Miguel, T, 1, A

comprobarPermisos(){
	permisos=$(id | cut -c 5)
	if [ "$permisos" -ne 0 ]
	then
		echo "Este script necesita privilegios de administracion"
		exit 1
	fi
}

anadirUsuarios(){
	while  read linea; 
		do
			nombre=$(echo "$linea" | cut -d ',' -f 1)
			password=$(echo "$linea" | cut -d ',' -f 2)
			nombreCompleto=$(echo "$linea" | cut -d ',' -f 3)
			
			if [  "$nombre" -a "$password" -a "$nombreCompleto" ]
			then	
				useradd "$nombre" -c "$nombreCompleto" -m -k /etc/skel -K UID_MIN=1815 -U 2> /dev/null
				if [ $? -eq 0 ]
				then
				
					usermod -aG 'sudo' "$nombre"
					passwd -x 30 "$nombre" &> /dev/null
					echo "$nombre:$password" | chpasswd
					echo "$nombreCompleto ha sido creado"
	
				else
					id=$(id -u "$nombre")
					echo "El usuario $nombre ya existe"
				fi
			else
				echo "Campo invalido"
	
			fi
		done < "$1"

}

eliminarUsuarios(){
	while read linea;
		do
			
			nombre=$(echo "$linea" | cut -d ',' -f 1)
			if [ "$nombre"  ]
			then
			
				id "$nombre" >/dev/null 2> /dev/null
				if [ $? -eq 0 ]
				then
					home=$(cat /etc/passwd | cut -d ':' -f 6 | grep -w "$nombre")
					tar -cf /extra/backup/"$nombre".tar "$home" 2> /dev/null
					if [ $? -eq 0 ]
					then	
						userdel -r "$nombre" 2> /dev/null
					
					fi	
				fi
			fi
		done < "$1"

}

comprobarPermisos
if [ $# -ne 2 ]
then
	echo "Numero incorrecto de parametros"
	exit 1
else
	if [ "$1" == "-a" ]
	then
		anadirUsuarios $2
	elif [ "$1" == "-s" ]
	then
		if [ ! -d /extra/backup ]
		then
			mkdir -p /extra/backup 2> /dev/null
		fi
		eliminarUsuarios $2
	else
		echo "Opcion invalida" >&2
		exit 1
	fi
fi
